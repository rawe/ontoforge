...
async def test_create_ontology_invalidates_runtime_schema_cache(mock_driver):
    ...
    with patch.object(runtime_service.repository, "get_full_schema", new=AsyncMock(return_value=schema)) as mock_get_schema:
        await runtime_service.get_full_schema("hr_view", mock_driver)
        await runtime_service.get_full_schema("hr_view", mock_driver)
