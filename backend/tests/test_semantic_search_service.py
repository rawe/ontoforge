...
async def test_search_disabled_raises(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded()), \
        ...
        with pytest.raises(ValidationError, match="EMBEDDING_PROVIDER"):
            await semantic_search("test", "engineers", "person", 10, None, store)


async def test_search_unknown_type_raises(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded()), \
        ...
        with pytest.raises(NotFoundError, match="nonexistent"):
            await semantic_search("test", "query", "nonexistent", 10, None, store)


async def test_search_single_type(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded()), \
         patch("ontoforge_server.runtime.service.get_embedding_provider", return_value=mock_provider), \
         patch("ontoforge_server.adapters.neo4j.runtime_queries.semantic_search", new_callable=AsyncMock) as mock_search:
        mock_search.return_value = search_results
        result = await semantic_search("test", "find Alice", "person", 10, None, store)
...
async def test_search_embed_failure_raises(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded()), \
        ...
        with pytest.raises(ValidationError, match="Failed to generate embedding"):
            await semantic_search("test", "query", "person", 10, None, store)


# --- No filters: limit passed directly (no over-fetch) ---


async def test_no_filters_passes_limit_directly(store):
...
async def test_equality_filter_no_overfetch(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded()), \
        ...
        await semantic_search(
            "test", "engineers", "person", 10, None, store,
...
async def test_operator_filter_passes_correct_clauses(store):
...
async def test_unknown_filter_property_raises(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded()), \
        ...
        with pytest.raises(ValidationError, match="Unknown filter property"):
            await semantic_search(
                "test", "query", "person", 10, None, store,
                filters={"nonexistent": "value"},
            )


async def test_contains_filter_rejected_on_semantic_search(store):
...
async def test_multiple_filters(store):
...
async def test_search_with_fields_projects_entities(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded()), \
        ...
        result = await semantic_search(
            "test", "find Alice", "person", 10, None, store,
            fields=["name"],
        )
...
async def test_search_without_fields_returns_all(store):
...
async def test_cross_type_search_uses_shared_index(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded(["person", "company"])), \
         ...
         patch("ontoforge_server.adapters.neo4j.runtime_queries.semantic_search", new_callable=AsyncMock) as mock_search:
        mock_search.return_value = search_results
        result = await semantic_search("test", "query", None, 10, None, store)
...
async def test_cross_type_search_scoped_overfetches_and_filters(store):
...
async def test_cross_type_search_truncates_to_limit(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=loaded), \
         ...
         patch("ontoforge_server.adapters.neo4j.runtime_queries.semantic_search", new_callable=AsyncMock) as mock_search:
        mock_search.return_value = search_results
        result = await semantic_search("test", "query", None, 2, None, store)

    assert result["total"] == 2


async def test_cross_type_search_rejects_filters(store):
    ...
    with patch("ontoforge_server.runtime.service._load_schema", return_value=_make_loaded()), \
        ...
        with pytest.raises(ValidationError, match="require 'type'"):
            await semantic_search(
                "test", "query", None, 10, None, store,
                filters={"location": "Berlin"},
            )


async def test_cross_type_search_fields_keeps_type_key(store):
...
async def test_cross_type_search_empty_scope_returns_empty(store):
