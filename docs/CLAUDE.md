...
8. **Technology-neutral above the adapter.** Everything in `docs/` except `storage-adapters.md` must hold for a reimplementation in another language. No library names, no file paths, no class or function names. One exception: a rule in `decisions.md` may name a vendor or a code constant when the rule itself is about it — a rule that binds one adapter, or pins one constant, names what it binds.
