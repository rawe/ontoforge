...
/** Session-scoped Q&A history per lens (survives tab switches, not reloads). */
...
function GeneratedQueryBlock({ lensKey, query }: { lensKey: string; query: string }) {
  ...
  return (
    <div className="rounded-md border">
      <div className="flex items-center gap-1 px-1.5 py-1">
        ...
        >
          <Link to={`/w/${lensKey}/query?query=${encodeURIComponent(query)}`}>
...
function AnswerCard({ lensKey, entry }: { lensKey: string; entry: AskEntry }) {
  return (
    <div className="rounded-lg border bg-card">
      ...
      <div className="space-y-3 px-4 py-3">
        ...
        {entry.response.query !== null && (
          <GeneratedQueryBlock lensKey={lensKey} query={entry.response.query} />
        )}
 ...
 */
export function AskTab({ lensKey }: { lensKey: string }) {
  const [question, setQuestion] = useState('')
  const [history, setHistory] = useState<AskEntry[]>(
    () => sessionHistory.get(lensKey) ?? [],
  )

  const ask = useMutation({
    mutationFn: (q: string) => aiQuery(lensKey, q),
    onSuccess: (response, q) => {
      const next = [{ question: q, response, at: Date.now() }, ...history]
      setHistory(next)
      sessionHistory.set(lensKey, next)
      setQuestion('')
    },
  })
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 py-4">
      ...
      <div className="mt-4 space-y-3">
        ...
        {latest !== undefined && <AnswerCard lensKey={lensKey} entry={latest} />}
        ...
        {previous.length > 0 && (
          <>
            ...
            {previous.map((entry) => (
              <AnswerCard key={entry.at} lensKey={lensKey} entry={entry} />
            ))}
          </>
        )}
      </div>
