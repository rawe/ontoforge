      ...
      <div className="flex items-center gap-1.5">
        ...
        <button
          ...
          onClick={(e) => {
            e.stopPropagation()
            void navigate(
              `/w/${lensKey}/explore?focus=${entity._entityTypeKey}:${entity._id}`,
            )
          }}
        >
...
function layout(entities: EntityInstance[], derivedEdges: DerivedEdge[], lensKey: string) {
 ...
 */
export function ResultsGraph({ lensKey, entities, edges }: ResultsGraphProps) {
  const { resolvedTheme } = useTheme()
  const graph = useMemo(
    () => layout(entities, edges, lensKey),
    [entities, edges, lensKey],
  )
