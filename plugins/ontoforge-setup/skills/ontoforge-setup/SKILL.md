...
1. **Ontology key** — the name/key for their ontology (e.g. `my_ontology`). Used in the MCP runtime URL and optionally as `DEFAULT_MCP_ONTOLOGY_KEY`.
...
- Set `DEFAULT_MCP_ONTOLOGY_KEY` if the user wants a default ontology key.
...
- Replace `my_ontology` in the runtime URL with the user's ontology key.
...
## OntoForge Environment Variables Reference

These are the **only** environment variables recognized by OntoForge. Do not invent others.
...
- **Runtime**: `http://<host>:8000/mcp/runtime/<ontology_key>` — data operations through an ontology lens

The ontology key in the runtime URL can alternatively be provided via the `X-Ontology-Key` HTTP header.
