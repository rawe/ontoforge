 ...
 * property of the related entity (`<relationTypeKey>.<propertyKey>`).
 *
 ...
 * entity type. What the lens hides fails exactly as what does not exist.
 * A resolved path becomes the port's `PathFilterCondition`; a fault is
 * returned, not thrown, so the filter parser can collect it with the
 * faults of every other key.
 */
...
/** The segment separator. No schema key may contain it, so a key that
 * does is a query path and never a property key. */
...
export function isQueryPath(key: string): boolean {
  return key.includes(PATH_SEPARATOR);
}
 ...
 * type the lens hides counts as unknown), relation type not touching the
 * listed type, ambiguous self-relation, unknown related property,
 * document-typed final property.
 */
export function resolveQueryPath(
  ...
  const propertyDef = relatedType.properties[propertyKey];
  if (propertyDef === undefined) {
    return {
      message: `Unknown filter property: '${propertyKey}' on related entity type '${relatedTypeKey}'`,
      detail:
        `Not defined in type '${relatedTypeKey}'. ` +
        `Property keys: ${keyList(Object.keys(relatedType.properties))}`,
    };
  }
  if (propertyDef.dataType === "document") {
    return {
      ...
      detail:
        `'${propertyKey}' on '${relatedTypeKey}' is a document property; ` +
